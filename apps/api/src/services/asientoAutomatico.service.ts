...
export async function asientoVenta(
  db: PrismaClient,
  empresaId: string,
  params: { fecha: Date; folio: number; neto: number; iva: number; total: number; tipo: string; glosa?: string | null },
): Promise<void> {
  ...
  const cClientes = await getCuenta(db, empresaId, '1.1.03');
  ...
  const lineas: { cuentaId: string; debe: Decimal; haber: Decimal; glosa: string | null }[] = [
    { cuentaId: cClientes.id, debe: new Decimal(total).toDecimalPlaces(4), haber: new Decimal(0), glosa: null },
...
export async function asientoCompra(
  db: PrismaClient,
  empresaId: string,
  params: { fecha: Date; folio: number; neto: number; iva: number; total: number; glosa?: string | null },
): Promise<void> {
  ...
  const cProveedores = await getCuenta(db, empresaId, '2.1.01');

  const lineas: { cuentaId: string; debe: Decimal; haber: Decimal; glosa: string | null }[] = [
    { cuentaId: cGasto.id, debe: new Decimal(neto).toDecimalPlaces(4), haber: new Decimal(0), glosa: null },
    { cuentaId: cProveedores.id, debe: new Decimal(0), haber: new Decimal(total).toDecimalPlaces(4), glosa: null },
  ];
