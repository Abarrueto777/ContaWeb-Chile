...
export async function seedPlanDeCuentas(empresaId: string, db: PrismaClient = prisma): Promise<void> {
  const creadas = new Map<string, string>(); // codigo -> id

  for (const cuenta of PLAN_DE_CUENTAS) {
    const cuentaPadreId = cuenta.codigoPadre ? creadas.get(cuenta.codigoPadre) : undefined;
...
async function main() {
  ...
  const admin = await prisma.usuario.upsert({
    where: { email: 'admin@contaweb.cl' },
    create: { email: 'admin@contaweb.cl', nombre: 'Administrador', password, rol: 'ADMIN' },
    update: {},
  });
  ...
  await seedPlanDeCuentas(empresa.id);

  console.log(`Seed completado — empresa: ${empresa.razonSocial} — ${PLAN_DE_CUENTAS.length} cuentas`);
}
